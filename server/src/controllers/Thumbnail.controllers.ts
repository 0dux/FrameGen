...
export const generateThumbnail = async (req: Request, res: Response) => {
  try {
    ...
    if (!response?.candidates || response.candidates.length === 0) {
      ...
      if (blockReason) {
        throw new Error(
          `Content blocked by AI safety filters. Reason: ${blockReason}. Try rephrasing your prompt or choosing a different style.`,
        );
      }
      throw new Error(
        "AI did not generate any content. Please try again with a different prompt.",
      );
    }
